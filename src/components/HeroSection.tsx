    ...
    <section
      ...
      <div className="w-full max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-12">
        ...
        <div className="flex-1 space-y-6 text-center md:text-left order-2 md:order-1">
          ...
          <h4 className="text-5xl md:text-7xl font-medium text-slate-700 dark:text-slate-200">
            I'm a{" "}
            <span className="text-indigo-400 font-semibold border-r-4 border-indigo-600 pr-2 animate-pulse">
              {roles[currentRoleIndex]}
            </span>
          </h4>

          <p className="text-2xl text-slate-200 dark:text-slate-200 max-w-2xl leading-relaxed mx-auto md:mx-0">
          ...
          <div className="flex items-center justify-center md:justify-start gap-8 pt-4">
            {[Instagram, Linkedin, Github].map((Icon, i) => (
              <a
                key={i}
                href="#"
                className="p-3.5 rounded-lg border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-200 hover:bg-indigo-600 hover:text-white transition-all duration-300 hover:-translate-y-1"
              >
                <Icon size={24} />
              </a>
            ))}
